...
class CrackUtils:
    ...
    def _rematch_crack_bboxes_to_boxes(self):
        ...
        for k, v in box_dict.items():
            try:
                ...
                keyed_boxes.append((str(k), np.array([xmin, ymin, xmax, ymax], float), area))
            except Exception:
                continue

        # --- Geometry extraction ---
        def _is_manual_crack(cr):
            src = str(cr.get("source", "")).strip().lower()
            return bool(cr.get("is_manual", False) or src in ("manual", "manual_poly"))

        def _geometry_points(cr):
            # Manual cracks -> trust full midline geometry when available.
            if _is_manual_crack(cr):
                mid = np.asarray(cr.get("midline", []), float)
                if mid.ndim == 2 and len(mid) >= 2:
                    return mid

            # Otherwise use user points if present.
            ups = cr.get("user_points", []) or []
            if ups:
                return np.asarray(ups, float)

            # Auto cracks -> do not use midline for coverage.
            return None

        def _endpoints(cr):
            # Prefer user-connected endpoints
            ups = cr.get("user_points", []) or []
            ...
            for conn in ucs:
                if isinstance(conn, (list, tuple)) and len(conn) >= 2:
                    ...
                    if 0 <= i0 < len(ups) and 0 <= i1 < len(ups):
                        return np.asarray(ups[i0], float), np.asarray(ups[i1], float)

            # Fallback: midline endpoints
            mid = np.asarray(cr.get("midline", []), float)
            if mid.ndim == 2 and len(mid) >= 2:
                return mid[0], mid[-1]
        ...
        # --- Containment test (vectorized) ---
        def _points_inside_box(points, bb):
        ...
        def _best_box_for_crack(cr):
            pts = _geometry_points(cr)
            p0, p1 = _endpoints(cr)

            if p0 is None or p1 is None or not keyed_boxes:
                return None

            # Endpoint-only fallback for cracks without trusted dense geometry.
            if pts is None:
                pts = np.vstack([p0, p1])

            best = None

            for k, bb, area in keyed_boxes:
                inside_mask = _points_inside_box(pts, bb)
                coverage = float(np.count_nonzero(inside_mask)) / float(len(pts))

                # Require endpoints inside for safety
                xmin, ymin, xmax, ymax = bb
                ep_inside = (
                ...
                if not ep_inside or coverage <= 0.0:
                    continue

                score = (coverage, -area)
                if best is None or score > best[0]:
                    best = (score, k, bb, coverage)

            if best is None:
                return None

            _, k, bb, cov = best
            return {"box_key": k, "bbox_xyxy": bb, "coverage": cov}
        ...
        for cid, cr in atomic.items():
            ...
            if old_bb != new_bb:
                ...
                print(
                    ...
                    f"coverage={m['coverage']:.2f} bbox {old_bb} -> {new_bb} (mask_crop reset)"
                )
            else:
                print(
                    f"[bbox rematch][atomic {cid}] box_key={m['box_key']} "
                    f"coverage={m['coverage']:.2f} bbox unchanged {new_bb}"
                )
        ...
        for ccid, cmb in combined.items():
            ...
            if not member_boxes:
                cmb["mask_bbox"] = None
                cmb["mask_crop"] = None
                print(f"[bbox rematch][combined {ccid}] UNMATCHED -> mask_bbox=None")
                continue
        ...
        print(f"[bbox rematch] atomic={len(atomic)} unmatched={unmatched} boxes={len(keyed_boxes)}")
    ...
    def select_folder(self):
        ...
        # ------------------------------------------------------------
        # Width baseline folder row (NEW)
        # ------------------------------------------------------------
        width_baseline_row = QHBoxLayout()
        width_baseline_label = QLabel("Width baseline folder:")
        ...
        # ------------------------------------------------------------
        # Segmentation/mask baseline folder row
        # ------------------------------------------------------------
        mask_baseline_row = QHBoxLayout()
        mask_baseline_label = QLabel("Mask baseline folder:")
        mask_baseline_folder_edit = QLineEdit(mask_baseline_folder_init)
        mask_baseline_browse_btn = QPushButton("Browse...")
        mask_baseline_row.addWidget(mask_baseline_label)
        mask_baseline_row.addWidget(mask_baseline_folder_edit)
        mask_baseline_row.addWidget(mask_baseline_browse_btn)
        layout.addLayout(mask_baseline_row)

        def update_baseline_rows():
            visible = use_baselines_checkbox.isChecked()
            for i in range(width_baseline_row.count()):
                w = width_baseline_row.itemAt(i).widget()
                if w:
                    w.setVisible(visible)
            for i in range(mask_baseline_row.count()):
                w = mask_baseline_row.itemAt(i).widget()
                if w:
                    w.setVisible(visible)

        use_baselines_checkbox.toggled.connect(update_baseline_rows)
        update_baseline_rows()
        ...
        mask_baseline_browse_btn.clicked.connect(
            lambda: mask_baseline_folder_edit.setText(
                QFileDialog.getExistingDirectory(dlg, "Select Mask Baseline Folder")
                or mask_baseline_folder_edit.text()
            )
        )
        ...
        while True:
            if dlg.exec_() == QDialog.Accepted:
                ...
                if use_baselines and not os.path.isdir(width_baseline_folder):
                    ...
                    continue
                if use_baselines and not os.path.isdir(mask_baseline_folder):
                    QMessageBox.critical(
                        dlg, "Error",
                        "Please select a valid mask baseline folder or uncheck 'Use Baselines'."
                    )
                    continue
                break
            else:
    ...
    def change_image(self):
        ...
        # ------------------------------------------------------------
        # Mask baseline folders (superfolder + method subfolders)
        # Expected layout: <mask_baseline_folder>/<method>/(...masks...)
        # ------------------------------------------------------------
        self.mask_baseline_superfolder = None
        self.mask_baseline_method_folders = tuple()
        self.mask_baseline_img_paths = {}
        if getattr(self, "use_baselines", False) and getattr(self, "mask_baseline_folder", ""):
            self.mask_baseline_superfolder = self.mask_baseline_folder
            self.mask_baseline_root = self.mask_baseline_folder
            try:
                method_dirs = []
                for d in os.listdir(self.mask_baseline_superfolder):
                    p = os.path.join(self.mask_baseline_superfolder, d)
                    if os.path.isdir(p):
                        method_dirs.append(p)
                self.mask_baseline_method_folders = tuple(sorted(method_dirs))
            except Exception:
                self.mask_baseline_method_folders = tuple()

            # Convenience map for current image: method_name -> probable mask path.
            # Supports both:
            # 1) <root>/<method>/<base_name>.png
            # 2) <root>/<method>/<base_name>/<base_name>.png (or .npy)
            exts = (".png", ".npy", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff")
            for method_dir in self.mask_baseline_method_folders:
                method_name = os.path.basename(method_dir)
                candidates = [os.path.join(method_dir, base_name + ext) for ext in exts]
                nested_dir = os.path.join(method_dir, base_name)
                if os.path.isdir(nested_dir):
                    candidates.extend([os.path.join(nested_dir, base_name + ext) for ext in exts])
    ...
    def select_end_points_manmidlines(self, metrics: bool = False):
        ...
        hint = QLabel(
            ...
            "Click unconnected point in Connection Mode to create new point.        Hover and click in respective mode to delete.        Mousewheel/2-finger swipe = zoom in/out.        T: toggle overlay.        M: move minimap (top-right/bottom-right).        Arrow keys: pan view (Shift = faster).\n"
