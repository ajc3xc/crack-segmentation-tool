...
OUTPUT_DIR = Path(r"C:\Users\13144\Documents\Masters_Thesis\datasets\SUT_Compressed\seg_baselines\hrsegnet")
WEIGHTS_PATH = Path(r"C:\Users\13144\Documents\Masters_Thesis\CURRENT_PROJECT_CODE\mask_baseline_generator\hrsegnet\pretrained_models\hrsegnetb32\best_model\model.pdparams")
...
def main():
    ...
    if paddle_device == "gpu" and not paddle.is_compiled_with_cuda():
        print("[HrSegNet] GPU requested but CUDA is unavailable. Falling back to CPU.")
        paddle_device = "cpu"
    ...
    print("[HrSegNet] Device: %s" % paddle_device)
    print("[HrSegNet] Output mode: %s" % OUTPUT_MODE)
    print("[HrSegNet] Variant: b32")
    print("[HrSegNet] Weights: %s" % WEIGHTS_PATH)
    print("[HrSegNet] Images: %d" % len(images))
    print("[HrSegNet] Output: %s" % OUTPUT_DIR)

    model = HrSegNetB32(in_channels=3, base=32, num_classes=2)
    ...
    print("[HrSegNet] Wrote timing CSV: %s" % per_image_csv)
    print("[HrSegNet] Wrote summary CSV: %s" % summary_csv)
    print("[HrSegNet] Export complete.")
