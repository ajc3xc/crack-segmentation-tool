...
class CrackAnnotator(QtWidgets.QWidget):
    ...
    def update_canvas_size(self):
        ...
        h = int(self.img_h * self.scale)
        self.setMinimumSize(w, h)
        self.resize(w, h)   # <- key for QScrollArea
        self.update()

    def resizeEvent(self, event):
        super().resizeEvent(event)
        # Auto-fit on first resize only
        if not hasattr(self, "_fit_done"):
            sa = self._find_scroll_area()
            if sa is not None and self.image_pixmap is not None:
                vp = sa.viewport()
                vw, vh = vp.width(), vp.height()
                if self.img_w > 0 and self.img_h > 0:
                    # Fit-to-view, allow scale < 1.0 so whole image is visible
                    fit_scale = min(vw / self.img_w, vh / self.img_h)
                    self.scale = fit_scale
                    self.update_canvas_size()
                    # Center the scrollbars
                    sa.horizontalScrollBar().setValue(
                        max(0, (self.width() - vw) // 2)
                    )
                    sa.verticalScrollBar().setValue(
                        max(0, (self.height() - vh) // 2)
                    )
            self._fit_done = True
    ...
    def wheelEvent(self, event):
        # Ctrl+Wheel → let QScrollArea handle scrolling normally
        if event.modifiers() & Qt.ControlModifier:
            super().wheelEvent(event)
            return

        # otherwise: Wheel = zoom
        f = 1.2 if event.angleDelta().y() > 0 else 1 / 1.2
        old = self.scale
        new = max(1.0, min(10.0, old * f))  # clamp min at 1.0 if you prefer
        if abs(new - old) < 1e-9:
            return
    ...
    # -------------------------------------------------------------------------
    # the rest of your class stays the same, except paintEvent top section:
    def paintEvent(self, event):
        qp = QPainter(self)
        qp.setRenderHint(QPainter.Antialiasing)
