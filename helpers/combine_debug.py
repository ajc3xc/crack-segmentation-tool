...
'''def auto_groups_from_atomic(annotation_dict_or_atomic,
    ...
    debug_rows = []
    # debug_root = where to save combine-debug diagnostics
    # save_folder is NOT defined in this scope, so DO NOT reference it
    ...
    for i in range(len(ids)):
        for j in range(i + 1, len(ids)):
            ...
            if image_hw is not None:
                ...
                # Save overlay if masks exist but don't overlap
                if maskA_px > 0 and maskB_px > 0 and not overlap:
                    vis = np.zeros((H, W, 3), np.uint8)
                    vis[..., 1] = np.clip(vis[..., 1] + (mA * 255), 0, 255)
                    vis[..., 2] = np.clip(vis[..., 2] + (mB * 255), 0, 255)
