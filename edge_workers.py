...
'''def plot_widths_colormap_on_crop(gt_vs_manual_rgb, e1, e2, track_e1=None, track_e2=None, out_png=None):
    """
    Continuous width-colored midline overlay on GT-vs-manual mask background,
    plus both geodesic edges (track_e1=red, track_e2=green).
    """
    ...
    # --- smooth widths to reduce jaggies
    widths_smooth = gaussian_filter1d(widths, sigma=2)

    # --- color mapping for width
    ...
    fig, ax = plt.subplots(figsize=(7, 7), dpi=320)
    ax.imshow(gt_vs_manual_rgb[..., ::-1], interpolation='bilinear')  # OpenCV BGR → RGB

    # --- continuous color midline
    for i in range(len(mid) - 1):
        x = [mid[i, 0], mid[i + 1, 0]]
        y = [mid[i, 1], mid[i + 1, 1]]
        ax.plot(x, y, color=colors[i], linewidth=2.4, alpha=0.97, solid_capstyle='round')

    # --- geodesic edges (if provided)
...
def edge_param_worker(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Therefore geometry MUST be scaled by the same 3× factor.
        S = 3.0

        if gt_vs_manual_overlay is not None:
            plot_widths_colormap_on_crop(
                gt_vs_manual_rgb = gt_vs_manual_overlay,     # 3× upscaled overlay
                e1               = normals_e1 * S,           # left edge sample points
                e2               = normals_e2 * S,           # right edge sample points
                midline_xy       = midline_xy_crop * S,      # midline points
                track_e1         = track_e1 * S,             # geodesic edge 1
                track_e2         = track_e2 * S,             # geodesic edge 2
                out_png          = widths_path
            )
        else:
            # fallback – no GT overlay, stay at native scale
            plot_widths_colormap_on_crop(
