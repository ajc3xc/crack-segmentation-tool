...
class CrackAnnotator(QtWidgets.QWidget):
    def __init__(self, image=None, boxes=None, initial_points=None, initial_connections=None, initial_midlines=None):
        ...
        # Timer for right-click delete hold
    ...
    def all_pairs_saturated(self):
        ...
        if n < 2:
            return True
        pairs = set(self.connections) | set(self.midlines.keys())
    ...
    def mousePressEvent(self, event):
        ...
        if self.polyline_mode:
            if event.button() == Qt.LeftButton:
                # Delete hovered connection/midline if not currently drawing
                ...
                # Start drawing from an existing point (do NOT delete the point)
                if not self._is_drawing:
                    if point_i is None:
                        return  # must click an existing point to start
        ...
        # --- Normal connection / point modes ---
        if event.button() == Qt.LeftButton:
            # ✅ Block deletion for read-only midlines & connections
            if (not self.polyline_mode) and self.connection_mode:
                if (mid_key is not None) and hasattr(self, "readonly_midlines") and mid_key in self.readonly_midlines:
                    return
                if (line_i is not None) and (point_i is None) and hasattr(self, "readonly_connections") and \
                self.connections[line_i] in self.readonly_connections:
                    return

            # Delete hovered midline in connection mode
    ...
    def mouseMoveEvent(self, event):
        ...
        # --- Manual midline mode drawing ---
        if self.polyline_mode and self._is_drawing and (event.buttons() & Qt.LeftButton):
            point_i = self._find_point_at(p)
            if point_i is not None and point_i != self._start_idx:
                # Auto-complete midline on hover
                key = self._sorted(self._start_idx, point_i)
                if (key not in self.midlines) and (key not in self.connections):
                    sx, sy = self.points[key[0]]
                    ex, ey = self.points[key[1]]
                    poly = [(float(sx), float(sy))] + self.polyline[1:-1] + [(float(ex), float(ey))]
                    self.midlines[key] = poly
    ...
    def paintEvent(self, event):
        ...
        for key, poly in self.midlines.items():
            if len(poly) < 2:
                continue
            # Only highlight in connection mode
            if self.connection_mode and key == self._hover_midline_key:
                thick = 8
            else:
                thick = 4
        ...
        # --- current manual midline being drawn ---
...
class CrackAnnotator(QtWidgets.QWidget):
    ...
    def mousePressEvent(self, event):
        ...
        # Prevent deletion of read-only items
        ...
        if self.polyline_mode:
            if event.button() == Qt.LeftButton:
                ...
                if not self._is_drawing:
                    ...
                    if point_i is not None and point_i != self._start_idx:
                        key = self._sorted(self._start_idx, point_i)
                        if (key in self.midlines) or (key in self.connections) or (key in self.readonly_connections):
                            self._last_pair_error = key
                            self.update()
                            return
                        if len(self.polyline) >= 2:
        ...
        if event.button() == Qt.LeftButton:
            ...
            if not self.connection_mode:
                if point_i is None:
                    ...
                    # prevent deleting read-only point
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.polyline_mode and self._is_drawing and (event.buttons() & Qt.LeftButton):
            ...
            if point_i is not None and point_i != self._start_idx:
                key = self._sorted(self._start_idx, point_i)
                if (key not in self.midlines) and (key not in self.connections) and (key not in self.readonly_connections):
                    sx, sy = self.points[key[0]]
                    ex, ey = self.points[key[1]]
                    poly = [(float(sx), float(sy))] + self.polyline[1:-1] + [(float(ex), float(ey))]
                    self.midlines[key] = poly
